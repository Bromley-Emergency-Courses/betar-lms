...
- Capacity-safe selection is implemented inside `submit_module_preferences(...)`. During submit/update it locks the selected and previously selected `module_offerings` rows, counts existing `enrolments` with status `planned` or `in_progress` plus other students' submitted choices for the same window/offering against `module_offerings.capacity`, excludes the current student's existing submission choices before checking replacements, and returns no submission ID for capacity-blocked attempts after writing `module_preference.capacity_blocked`.
- `module_preference_window_offering_capacity(...)` provides the staff and student capacity snapshot for configured window offerings: capacity, already enrolled, preference selections, remaining places, full, and unavailable. Staff see these counters in `/admissions/preferences`; students see full offerings as unavailable in `/portal/module-preferences` unless they are keeping their own current saved choice.
...
- Audit events are written for `module_preference_window.created`, `module_preference_window.opened`, `module_preference_window.closed`, `module_preference_window.confirmed`, `module_preference.submitted`, `module_preference.updated`, and `module_preference.capacity_blocked`.
...
- This slice deliberately does not implement waitlists, mandatory-module enforcement/override, reminder emails, low-uptake cancellation, enrolment creation, finance rows, invoices, payments, real email sends, or admissions registration/conversion behavior changes.
