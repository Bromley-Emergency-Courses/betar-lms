...
- [x] Build the feature-flagged shared Admissions overview, local navigation, table/query/selection primitives, drawers, and durable batch results.
- [ ] Build the complete new-student operations workspace and full-record actions.
  - [x] Add the lead-based full record for every stage, new-enquiry entry, administrative details, complete application facts, study plan, evidence decisions and override, structured correction authoring/review/history, readiness, offer/rejection, restricted support-needs reveal, registration reopen/conversion, correspondence, and audit history.
  - [x] Add sanctioned offer reissue/withdrawal and abandoned pre-submission close/reopen records, with guarded mutations and preserved reason, audit, correspondence, and prior-state history rather than direct status editing.
  - [ ] Add contextual individual/bulk reminder, invitation, abandonment and deadline actions through the reviewed batch executor rather than synchronous cohort work.
    - [x] Connect one/selected/all-matching invitation and pre-submission abandonment review, mixed-eligibility preview, durable background execution, execution-time rechecks, interrupted-target recovery, per-record results, and failure-only retry.
    - [ ] Add contextual offer/registration reminders and explicit overdue offer/registration lapse actions to the same executor.
...
- [ ] Complete allowlisted fake-record new-applicant and returning-student pilots and obtain deliberate product-owner approval.
  - [x] Add admin-only applicant and returning-student pilot-marker controls with allowlist/configuration visibility, explicit fake-data confirmation, reasoned audited removal, and safe person linking for unlinked fake enquiries without issuing an invitation.
...
| 2026-08-09 | `12amathew/review-admissions-workflow` | Added the feature-flagged shared staff UX foundation from validated Variant A: a two-workspace Admissions gateway with email-safety and cross-workflow exception visibility; stable local navigation; separate new- and returning-student operations routes; staff-only operations projections; URL-backed search/filter/pagination; visible-page, cross-page selected, and deliberate all-matching scope; accessible quick-dossier drawers; and a durable batch progress/per-target results route with active refresh. The existing staff surfaces remain the default until the coordinated cutover flag is deliberately enabled. Contextual batch actions, complete full-record routes, the background worker, and returning response/confirmation records remain later slices. | `npm run lint`; `npm run test` (29 files, 177 tests); `npm run build`; feature-flagged route smoke checks; executable PGlite projection/contact-state coverage. |
| 2026-08-09 | `12amathew/review-admissions-workflow` | Added the feature-flagged lead-based new-student full record and connected every workspace row to it: pre-application enquiry entry and invitation, administrative details without stage mutation, complete application/study-plan facts, evidence decisions and reasoned override, structured correction authoring/resubmission review/cancellation/history, readiness, guarded offer/rejection, deliberately audited restricted support-needs reveal, registration reopen/conversion, correspondence and redacted audit history. Existing actions now return to the replacement record when invoked there. The slice also fixes workspace routes that could otherwise cache a flag-off redirect by forcing request-time evaluation. The sanctioned offer reissue/withdrawal, abandoned close/reopen, and reviewed batch executor remain explicit gaps, so incomplete actions are not simulated in the UI. | `npm run lint`; `npm run test` (30 files, 181 tests); `npm run build`; feature-flagged production-route smoke checks. |
| 2026-08-09 | `12amathew/review-admissions-workflow` | Added sanctioned individual closure actions to the new-student full record: reversible, non-archival pre-submission abandonment with restoration to the prior lead stage; lapsed-offer reissue with a new future deadline and immutable prior lapse/deadline history; and terminal issued/lapsed offer withdrawal. Every transition is admin-guarded, reasoned, audited, projected from authoritative records, and—where applicant-facing—creates a log-first correspondence attempt under existing pilot/live controls. | `npm run lint`; `npm run test` (31 files, 188 tests); `npm run build`; executable PGlite abandonment/reopen/reissue/withdrawal/projection/correspondence coverage. |
| 2026-08-09 | `12amathew/review-admissions-workflow` | Connected the first reviewed executor actions to the new-student operations table: one/selected/all-matching invitation and reversible abandonment; explicit reviewed scope; editable invitation snapshot with transient non-persisted secure-link insertion; mixed eligible/excluded preview; server-side pilot/live safety; transactional person linking for previously unlinked enquiries; immediate durable enqueue with post-response bounded execution; execution-time workflow rechecks; stale-running recovery; persistent progress/results; and failure-only retry that preserves the reviewed reason/message. Reminder and explicit deadline-lapse actions remain the next executor slice. | `npm run lint`; `npm run test` (32 files, 195 tests); `npm run build`; executable PGlite 500-record enqueue/person-link/log-link/recovery/retry/permission coverage. |
| 2026-08-09 | `12amathew/review-admissions-workflow` | Made controlled email-pilot setup usable without SQL: admin-only pilot-marker controls now appear on the full new-applicant record and returning-student dossier; show marker, current-address allowlist, delivery/configuration state; require explicit fake-data confirmation, label and reason; support reasoned audited removal; and safely create/link a person for an unlinked fake enquiry without issuing an invitation, changing its journey stage or recording contact. | `npm run lint`; `npm run test -- --maxWorkers=1` (33 files, 197 tests); `npm run build`; executable PGlite unlinked-enquiry/marker/stage/audit/permission coverage. |
| 2026-08-09 | `12amathew/review-admissions-workflow` | Corrected application invitation nonce hashing on Supabase by making both security-definer invitation functions resolve the `pgcrypto` extension schema, and replaced the invitation batch's silent zero-eligible no-op with an explicit blocking explanation, a truthful `No eligible records` label, and visible disabled styling. | `npm run lint`; `npm run test -- --maxWorkers=1`; `npm run build`; migration applied and function search paths verified against local Supabase without resetting test data. |
| 2026-08-10 | `12amathew/review-admissions-workflow` | Corrected applicant authentication emails to send an SSR-readable app callback containing Supabase's one-time hashed token instead of the default fragment-based action link, while preserving the invitation ID and claim nonce that bind first access to the intended applicant. Added first-access guidance for the deliberately non-enumerating sign-in request screen. Previously issued links must be replaced with fresh invitations. | `npm run lint`; `npm run test -- --maxWorkers=1`; `npm run build`. |
| 2026-08-10 | `12amathew/review-admissions-workflow` | Integrated pilot feedback across the applicant and registration journeys: explained single-use sign-in links and the fresh-link return path; added red required markers, native validation and in-page submit failures; connected structured correction requests to immediate controlled magic-link email delivery; raised authenticated upload request limits to match storage validation; exposed registration documents and staff verification controls on the full record; and added auditable applicant/staff upload-or-induction verification routes without creating placeholder files. | `supabase migration up --local --yes`; `npm run lint`; `npm run test -- --maxWorkers=1`; `npm run build`. |
| 2026-08-10 | `12amathew/review-admissions-workflow` | Completed the applicant-facing correction workspace for submitted applications: active requests now expose only the specifically requested field or evidence responses, preserve the original submission, support saved drafts and replacement uploads, show completion progress, and lock again after deliberate resubmission for staff review. | `npm run lint`; `npm run test -- --maxWorkers=1`; `npm run build`. |
