...
- [ ] Build the complete new-student operations workspace and full-record actions.
  ...
  - [ ] Add sanctioned offer reissue/withdrawal and abandoned pre-submission close/reopen records; the projection currently recommends reissue without a matching mutation, so the UI blocks direct status editing and identifies the gap.
