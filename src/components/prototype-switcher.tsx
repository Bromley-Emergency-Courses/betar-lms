...
export function PrototypeSwitcher({ variants, current }: { variants: PrototypeVariant[]; current: string }) {
  ...
  return <div className="prototype-switcher" role="group" aria-label="Prototype variants">
    <button type="button" onClick={() => move(-1)} aria-label="Previous prototype variant"><ArrowLeft size={16} /></button>
    <span><strong>{active.key}</strong> — {active.name}</span>
    <button type="button" onClick={() => move(1)} aria-label="Next prototype variant"><ArrowRight size={16} /></button>
  </div>;
}
